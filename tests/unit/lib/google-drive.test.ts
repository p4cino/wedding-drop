...
describe("Google Drive Helper & Security Tests", () => {
  ...
  describe("Kryptograficzny stan HMAC (State Security)", () => {
    ...
    it("powinien odrzucić stan ze zmodyfikowanym payloadem (tampered HMAC)", () => {
      ...
      const fakePayload = JSON.stringify({ slug: "hacked-slug", ts: Date.now(), nonce: "123" });
    ...
    it("powinien odrzucić przeterminowany stan (>15 minut)", () => {
      ...
      const payload = JSON.stringify({ slug: "test-slug", ts: oldTimestamp, nonce: "abc" });
      const hmac = crypto.createHmac("sha256", secret).update(payload).digest("hex");
      const stateObj = { payload: Buffer.from(payload).toString("base64url"), hmac };
      const expiredState = Buffer.from(JSON.stringify(stateObj)).toString("base64url");
