...
describe("LightboxModal Component", () => {
  ...
  it("nie powinien nic renderować, gdy currentIndex === null", () => {
    const { container } = render(
      <LightboxModal items={mockItems} currentIndex={null} onClose={vi.fn()} onNavigate={vi.fn()} />
    );
  ...
  it("powinien wyświetlać podgląd zdjęcia dla currentIndex = 0 i pozwalać na pobranie", () => {
    ...
    render(
      <LightboxModal items={mockItems} currentIndex={0} onClose={onClose} onNavigate={onNavigate} />
    );
  ...
  it("powinien renderować wideo dla elementu o fileType === 'video'", () => {
    const { container } = render(
      <LightboxModal items={mockItems} currentIndex={1} onClose={vi.fn()} onNavigate={vi.fn()} />
    );
  ...
  it("powinien obsłużyć ArrowLeft gdy currentIndex = 1", () => {
    const onNavigate = vi.fn();
    render(
      <LightboxModal items={mockItems} currentIndex={1} onClose={vi.fn()} onNavigate={onNavigate} />
    );
  ...
  it("powinien nawigować przy geście swipe left na ekranie dotykowym", () => {
    const onNavigate = vi.fn();
    const { container } = render(
      <LightboxModal items={mockItems} currentIndex={0} onClose={vi.fn()} onNavigate={onNavigate} />
    );
