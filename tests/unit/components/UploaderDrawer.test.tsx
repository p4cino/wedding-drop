...
vi.mock("tus-js-client", () => ({
  Upload: vi.fn().mockImplementation(function (this: any, file: any, options: any) {
    return {
      start: () => {
        if (shouldFailUpload) {
...
describe("UploaderDrawer Component", () => {
  ...
  it("nie powinien renderować niczego, gdy isOpen === false", () => {
    const { container } = render(
      <UploaderDrawer gallerySlug="kasia-i-tomek" isOpen={false} onClose={vi.fn()} />
    );
  ...
  it("powinien renderować formularz, obsługiwać wybór pliku, upload wideo i zamykanie", async () => {
    ...
    // Wpisanie podpisu
    const nameInput = screen.getByPlaceholderText(/np. Ciocia Kasia i Wujek Michał/i);
    fireEvent.change(nameInput, { target: { value: "Wujek Zdzichu" } });
    expect(nameInput).toHaveValue("Wujek Zdzichu");

    // Wybór wideo
    const file = new File(["test-content"], "taniec.mp4", { type: "video/mp4" });
    ...
    const uploadBtn = screen.getByRole("button", { name: /Wyślij do galerii/i });
    ...
    const doneBtn = screen.getByRole("button", { name: /Gotowe, wróć do galerii/i });
