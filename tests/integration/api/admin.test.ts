...
import { POST as adminHandler, generateAdminToken } from "@/app/api/admin/route";
import { NextRequest } from "next/server";

vi.mock("bcryptjs", () => ({
  default: {
    compare: vi.fn().mockImplementation((pwd) => Promise.resolve(pwd === "admin123")),
          ...
          return {
            where: vi.fn(() => ({
              limit: vi.fn().mockImplementation(() => {
                ...
                if (existingSlugMatches) return Promise.resolve([mockGalleries[0]]);
...
describe("Admin API Route", () => {
  ...
  it("powinien odrzucić żądanie ze sfałszowanym tokenem admina", async () => {
    ...
    const res = await adminHandler(req);
    expect(res.status).toBe(401);
    expect(await res.json()).toEqual({ error: "Brak uprawnień administratora" });
  });
