...
vi.mock("bcryptjs", () => ({
  default: {
    compare: vi.fn().mockImplementation((pwd) => Promise.resolve(pwd === "prawidloweHaslo")),
...
let mockExistingCard: any[] = [{ id: "card-1" }];
  ...
  return {
    db: {
      select: vi.fn(() => ({
        from: vi.fn(() => ({
          where: vi.fn(() => ({
            limit: vi.fn().mockImplementation(() => Promise.resolve(mockGalleryList)),
          })),
        })),
      })),
      update: vi.fn(() => ({
