...
export async function ensureDriveFolder(
	...
	if (!createRes.data.id) {
		throw new Error(
			`Nie udało się utworzyć folderu '${folderName}' na Dysku Google`,
		);
	}
...
export async function uploadFileToDrive(
	...
	while (attempt < maxRetries) {
		...
		try {
			...
			if (!res.data.id) {
				throw new Error(
					`Brak ID w odpowiedzi Google Drive dla pliku ${fileName}`,
				);
			}
			...
			console.error(
				`Błąd przesyłania pliku ${fileName} (próba ${attempt}/${maxRetries}):`,
			...
			if (
				...
				throw new Error(
					"Brak miejsca na Twoim koncie Google Drive (storageQuotaExceeded)",
				);
			}
	...
	throw new Error(
		`Niepowodzenie przesyłania pliku ${fileName} po ${maxRetries} próbach`,
	);
}
