...
import { initTusServer } from "./src/lib/tus-server";
import { recoverInterruptedExports } from "./src/lib/gdrive-exporter";
...
async function bootstrap() {
  ...
  const server = createServer((req, res) => {
    ...
    if (url.startsWith("/media-file/")) {
      const relativePath = decodeURIComponent(url.replace(/^\/media-file\//, "").split("?")[0]);
  ...
  server.listen(port, hostname, () => {
    console.log(`> Serwer WeddingDrop działa pod adresem: http://${hostname}:${port}`);
