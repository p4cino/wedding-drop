...
const MOCK_MEDIA = [
  {
    ...
    fileSize: 1024000,
    thumbUrl: "data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='200' height='200'><rect width='200' height='200' fill='goldenrod'/><text x='20' y='100' fill='white'>Foto 1</text></svg>",
    rawUrl: "data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='800' height='600'><rect width='800' height='600' fill='goldenrod'/></svg>",
  ...
  },
  {
    ...
    fileSize: 2048000,
    thumbUrl: "data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='200' height='200'><rect width='200' height='200' fill='darkred'/><text x='20' y='100' fill='white'>Foto 2</text></svg>",
    rawUrl: "data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='800' height='600'><rect width='800' height='600' fill='darkred'/></svg>",
    createdAt: "2026-09-12T15:30:00.000Z",
  },
];

test.describe("Ścieżka Gościa Weselnego (Mobile & Desktop)", () => {
  test("UC1: powinien załadować widok galerii gościa z nagłówkiem i statystykami", async ({ page }) => {
    ...
    const uploadBtn = page.getByRole("button", { name: /Dodaj zdjęcia i filmy/i });
    await expect(uploadBtn).toBeVisible();
  });

  test("UC2: powinien wyświetlić czytelny stan pusty, gdy w galerii nie ma jeszcze zdjęć", async ({ page }) => {
    ...
    await page.goto("/g/kasia-i-tomek");
    await expect(page.getByText("Galeria czeka na pierwsze zdjęcia!")).toBeVisible();
    await expect(
      page.getByText("Bądź pierwszą osobą, która uwieczni ten wyjątkowy dzień")
    ).toBeVisible();
  });

  test("UC3: powinien otworzyć drawer uploadu, umożliwić wpisanie podpisu i zamknąć go", async ({ page }) => {
    await page.goto("/g/kasia-i-tomek");
    ...
    await expect(page.getByRole("heading", { name: "Dodaj zdjęcia i filmy" })).toBeVisible();
    await expect(page.getByText("Bez logowania • Zostaną zapisane w galerii")).toBeVisible();
    ...
    await expect(page.getByText("Kliknij, aby wybrać z galerii lub aparatu")).toBeVisible();
    ...
    await expect(page.getByText("Bez logowania • Zostaną zapisane w galerii")).not.toBeVisible();
  });

  test("UC4: powinien wyrenderować siatkę ze zdjęciami i podpisami autorów", async ({ page }) => {
    await page.route("**/api/gallery/kasia-i-tomek/media", async (route) => {
      await route.fulfill({
        status: 200,
        contentType: "application/json",
  ...
  test("UC5: powinien otworzyć pełnoekranowy Lightbox i nawigować przyciskami oraz klawiaturą", async ({ page }) => {
    await page.route("**/api/gallery/kasia-i-tomek/media", async (route) => {
      await route.fulfill({
        status: 200,
        contentType: "application/json",
  ...
  test("UC6: powinien obsługiwać gesty dotykowe Swipe (przesuwanie palcem) w Lightboxie", async ({ page }) => {
    await page.route("**/api/gallery/kasia-i-tomek/media", async (route) => {
      await route.fulfill({
        status: 200,
        contentType: "application/json",
    ...
    await page.evaluate(() => {
      ...
      const fire = (type: string, x: number) => {
        ...
        Object.defineProperty(ev, "targetTouches", { value: [{ clientX: x, clientY: 200 }] });
        el.dispatchEvent(ev);
      };
      fire("touchstart", 300);
      fire("touchmove", 100);
  ...
  test("UC7: powinien zawierać przycisk pobierania pojedynczego zdjęcia z Lightboxa", async ({ page }) => {
    await page.route("**/api/gallery/kasia-i-tomek/media", async (route) => {
      await route.fulfill({
        status: 200,
        contentType: "application/json",
    ...
    await expect(downloadBtn).toHaveAttribute("download", "pierwszy_taniec.jpg");
  });
});
