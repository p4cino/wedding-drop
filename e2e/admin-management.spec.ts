...
test.describe("Panel Administratora", () => {
  ...
  test("UC1: powinien zalogować admina poprawnymi danymi i wyświetlić panel zarządzania", async ({ page }) => {
  ...
  test("UC2: powinien odrzucić błędne hasło administratora i wyświetlić komunikat błędu", async ({ page }) => {
    await page.locator('input[type="text"]').fill("admin");
  ...
  test("UC3: powinien otworzyć i zamknąć modal tworzenia nowego wesela", async ({ page }) => {
    // Logowanie
    await page.locator('input[type="text"]').fill("admin");
    await page.locator('input[type="password"]').fill("admin123");
    await page.getByRole("button", { name: "Zaloguj się" }).click();
  ...
  test("UC4: powinien pomyślnie utworzyć nowe wesele i wyświetlić je na liście", async ({ page }) => {
    ...
    await page.getByPlaceholder("kontakt@kasiaitomek.pl").fill(`${uniqueSlug}@example.com`);
    await page.getByPlaceholder("Hasło do moderacji i pobierania ZIP").fill("sekret123");

    // Wysłanie formularza
    await page.getByRole("button", { name: /Utwórz wesele/i }).click();

    // Weryfikacja ekranu sukcesu
    await expect(page.getByText("Galeria została pomyślnie utworzona!")).toBeVisible();
    await expect(page.getByText(`Slug: ${uniqueSlug}`)).toBeVisible();

    // Zamknięcie modalu i sprawdzenie obecności w tabeli
    await page.getByRole("button", { name: "Zamknij" }).click();
    await expect(page.getByRole("cell", { name: uniqueSlug, exact: true })).toBeVisible();
    await expect(page.getByRole("cell", { name: couple, exact: true })).toBeVisible();
  });

  test("UC5: powinien bezpiecznie rozwiązać kolizję sluga poprzez dodanie losowego sufiksu", async ({ page }) => {
    // Logowanie
    await page.locator('input[type="text"]').fill("admin");
    await page.locator('input[type="password"]').fill("admin123");
    await page.getByRole("button", { name: "Zaloguj się" }).click();
    ...
    await page.getByPlaceholder("kontakt@kasiaitomek.pl").fill("kolejni@example.com");
    await page.getByPlaceholder("Hasło do moderacji i pobierania ZIP").fill("sekret123");
    ...
    await expect(page.getByText("Galeria została pomyślnie utworzona!")).toBeVisible();
    await expect(page.locator("text=/Slug: kasia-i-tomek-[a-z0-9]+/")).toBeVisible();

    await page.getByRole("button", { name: "Zamknij" }).click();
    await expect(page.locator(`text=${coupleName}`)).toBeVisible();
  });

  test("UC6: powinien umożliwić usunięcie galerii po potwierdzeniu dialogu", async ({ page }) => {
    ...
    await page.getByPlaceholder("kontakt@kasiaitomek.pl").fill("del@example.com");
    await page.getByPlaceholder("Hasło do moderacji i pobierania ZIP").fill("sekret123");
    await page.getByRole("button", { name: /Utwórz wesele/i }).click();
    await page.getByRole("button", { name: "Zamknij" }).click();
