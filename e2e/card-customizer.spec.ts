...
test.describe("Generator i Edytor Karteczki A6", () => {
  ...
  test("UC1: powinien załadować widok projektanta z podglądem karty A6 i kodem QR", async ({ page }) => {
  ...
  test("UC2: powinien umożliwić przełączanie motywów kolorystycznych i weryfikację stylów", async ({ page }) => {
    // 1. Zmiana na Butelkowa Zieleń (#1B4332 / #D4AF37)
    const emeraldPalette = page.getByRole("button", { name: /Butelkowa Zieleń/i });
    await expect(emeraldPalette).toBeVisible();
    await emeraldPalette.click();

    const textInput = page.locator('div:has(> label:has-text("Kolor tekstu i QR")) input[type="text"]');
    const frameInput = page.locator('div:has(> label:has-text("Kolor złotej ramki")) input[type="text"]');
  ...
  test("UC3: powinien aktualizować nagłówek i instrukcję w czasie rzeczywistym na podglądzie", async ({ page }) => {
    const printableCard = page.locator("#printable-card");
    const headlineInput = page.locator('div:has(> label:has-text("Nagłówek")) input');
    ...
    await textarea.fill("Krok 1: Skieruj aparat na kod\nKrok 2: Ciesz się wspomnieniami!");
    await expect(printableCard).toContainText("Krok 1: Skieruj aparat na kod");
    await expect(printableCard).toContainText("Krok 2: Ciesz się wspomnieniami!");
  });

  test("UC4: powinien generować link do pobrania PDF zawierający parametry konfiguracyjne w query stringu", async ({ page }) => {
    const headlineInput = page.locator('div:has(> label:has-text("Nagłówek")) input');
    await expect(headlineInput).toBeVisible();
    ...
    await expect(pdfBtn).toHaveAttribute(
      "href",
      new RegExp("primaryColor=%230F172A.*headline=Wielkie%20Wesele")
    );
  });

  test("UC5: powinien przenieść użytkownika z powrotem do galerii po kliknięciu linku", async ({ page }) => {
