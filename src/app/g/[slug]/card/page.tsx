...
import { ArrowLeft, Download, Printer, Palette, Sparkles, Check } from "lucide-react";
...
export default function CardCustomizerPage() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (data.cardSettings) {
            if (data.cardSettings.headline) setHeadline(data.cardSettings.headline);
            if (data.cardSettings.customInstructions) setInstructions(data.cardSettings.customInstructions);
            if (data.cardSettings.primaryColor) setPrimaryColor(data.cardSettings.primaryColor);
            if (data.cardSettings.accentColor) setAccentColor(data.cardSettings.accentColor);
          }
        }
      } catch (e) {
  ...
  useEffect(() => {
    const url = typeof window !== "undefined" ? `${window.location.origin}/g/${slug}` : `http://localhost:3000/g/${slug}`;
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#FAF8F5]">
        <div className="text-center space-y-3">
          ...
          <p className="text-sm font-medium text-slate-600">Ładowanie projektu winietki...</p>
        </div>
      </div>
    );
  }
      ...
      <main className="max-w-5xl mx-auto px-4 py-8 grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="no-print lg:col-span-5 space-y-6 bg-white p-6 rounded-3xl border border-slate-200/80 shadow-xs">
          <div>
            ...
            <h2 className="font-serif-luxury text-2xl font-bold text-slate-900">Karteczka na Stolik (A6)</h2>
            <p className="text-xs text-slate-500 mt-1">
              Wydrukuj i postaw na stołach weselnych, by goście mogli błyskawicznie dodać swoje zdjęcia.
            </p>
          </div>
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-2 p-2 rounded-xl border text-xs font-medium transition text-left ${
                      isActive ? "border-amber-600 bg-amber-50/50" : "border-slate-200 hover:border-slate-300"
                    }`}
                  >
                    <div className="flex -space-x-1 shrink-0">
                      <span className="w-4 h-4 rounded-full border border-white" style={{ background: palette.primary }} />
                      <span className="w-4 h-4 rounded-full border border-white" style={{ background: palette.accent }} />
                    </div>
                    <span className="truncate text-slate-800">{palette.name}</span>
                    {isActive && <Check className="w-3.5 h-3.5 text-amber-600 ml-auto shrink-0" />}
                  </button>
                );
              })}
            </div>
          ...
          <div className="grid grid-cols-2 gap-3 pt-1">
            <div>
              <label className="block text-[11px] font-medium text-slate-500 mb-1">Kolor tekstu i QR</label>
              <div className="flex items-center gap-2">
                <input
                  type="color"
                  value={primaryColor}
            ...
            <div>
              <label className="block text-[11px] font-medium text-slate-500 mb-1">Kolor złotej ramki</label>
              <div className="flex items-center gap-2">
                <input
                  type="color"
                  value={accentColor}
          ...
          >
            ...
            <div className="relative z-10 my-auto flex flex-col items-center">
              <div className="p-2.5 bg-white rounded-2xl shadow-xs border border-slate-100">
                {qrDataUrl ? (
                  <img src={qrDataUrl} alt="Kod QR Wesela" className="w-36 h-36 sm:w-40 sm:h-40 object-contain" />
                ) : (
