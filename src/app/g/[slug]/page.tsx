...
import { Heart, Plus, Sparkles, RefreshCw, QrCode, Image as ImageIcon, Video } from "lucide-react";
import UploaderDrawer from "@/components/UploaderDrawer";
import MediaGrid from "@/components/MediaGrid";
import LightboxModal, { MediaItemData } from "@/components/LightboxModal";
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#FAF8F5]">
        <div className="text-center space-y-3">
          ...
          <p className="font-serif-luxury text-lg text-slate-700">Wczytywanie galerii ślubnej...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#FAF8F5] p-6 text-center">
        <div className="max-w-md bg-white p-8 rounded-3xl shadow-sm border border-slate-200">
          <h2 className="font-serif-luxury text-2xl font-bold text-slate-900 mb-2">Galeria nie została znaleziona</h2>
          <p className="text-sm text-slate-500 mb-6">Upewnij się, że adres URL lub kod QR jest prawidłowy.</p>
          <Link
            href="/"
  ...
  return (
    <div className="min-h-screen pb-28 bg-[#FAF8F5]">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 pt-6">
        <MediaGrid items={items} onItemClick={(index) => setLightboxIndex(index)} />
      </main>
