...
import { db } from "@/db";
import { admins, galleries, mediaItems, cardSettings } from "@/db/schema";
...
function getAdminSecret(): string {
  return process.env.ADMIN_SECRET || process.env.ADMIN_PASSWORD || "wedding-admin-secret-fallback-key";
}
...
export function verifyAdminToken(token: string | null | undefined): boolean {
  ...
  if (isNaN(timestamp) || Date.now() - timestamp > maxAge || timestamp > Date.now() + 60000) {
    return false;
  }
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (action === "create-gallery") {
      const { coupleNames, weddingDate, ownerEmail, ownerPassword, customSlug, accessPin, maxStorageGb } = body;

      if (!coupleNames || !weddingDate || !ownerEmail || !ownerPassword) {
        return NextResponse.json({ error: "Wszystkie podstawowe pola są wymagane" }, { status: 400 });
      }
      ...
      const existingSlug = await db.select().from(galleries).where(eq(galleries.slug, slug)).limit(1);
      if (existingSlug.length) {
        slug = `${slug}-${Math.random().toString(36).substring(2, 6)}`;
      }

      const ownerPasswordHash = await bcrypt.hash(ownerPassword, 10);
      const maxStorageBytes = maxStorageGb ? parseInt(maxStorageGb, 10) * 1024 * 1024 * 1024 : 0;
      ...
      await db.insert(cardSettings).values({
        ...
        subheadline: "Zeskanuj kod QR aparatem w telefonie i dodaj swoje zdjęcia z naszego wesela",
        primaryColor: "#1E293B",
        accentColor: "#D4AF37",
        paperSize: "A6",
      });
      ...
      await fs.mkdir(path.join(dataDir, "galleries", slug, "raw"), { recursive: true });
      await fs.mkdir(path.join(dataDir, "galleries", slug, "thumbs"), { recursive: true });
    ...
    if (action === "delete-gallery") {
      ...
      const target = await db.select().from(galleries).where(eq(galleries.id, galleryId)).limit(1);
      ...
      if (target.length) {
        ...
        await fs.rm(path.join(dataDir, "galleries", slug), { recursive: true, force: true }).catch(() => {});
      }
