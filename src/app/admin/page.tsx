...
import { ShieldCheck, Plus, Trash2, ExternalLink, QrCode, HardDrive, Calendar, Users, X, Check } from "lucide-react";
...
export default function AdminDashboardPage() {
  ...
  const [weddingDate, setWeddingDate] = useState(new Date().toISOString().slice(0, 10));
  ...
  const handleDelete = async (galleryId: string, slug: string) => {
    if (!confirm(`Czy na pewno chcesz bezpowrotnie usunąć galerię "${slug}" wraz ze wszystkimi zdjęciami?`)) return;
    if (!token) return;
  ...
  const totalGlobalFiles = galleries.reduce((acc, g) => acc + (g.totalFiles || 0), 0);
  const totalGlobalBytes = galleries.reduce((acc, g) => acc + Number(g.totalBytes || 0), 0);
    ...
    <div className="min-h-screen bg-[#FAF8F5] pb-20">
      ...
      <header className="bg-white border-b border-slate-200 px-6 py-4 sticky top-0 z-30 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-bold text-slate-900 text-base">Zarządzanie WeddingDrop</h1>
            <p className="text-xs text-slate-400">Panel Administratora Systemu</p>
          </div>
        </div>
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs">
            ...
            <p className="text-3xl font-bold text-slate-900">{galleries.length}</p>
          </div>
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs">
            ...
            <p className="text-3xl font-bold text-slate-900">{totalGlobalFiles}</p>
          </div>
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs">
            ...
            <p className="text-3xl font-bold text-slate-900">{totalGlobalMb} MB</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            <h3 className="font-bold text-slate-800 text-sm">Aktywne Galerie Weselne</h3>
            <span className="text-xs text-slate-400">{galleries.length} rekordów</span>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-slate-100">
                {galleries.map((g) => {
                  const mb = (Number(g.totalBytes || 0) / (1024 * 1024)).toFixed(1);
                  return (
                    <tr key={g.id} className="hover:bg-slate-50/80 transition">
                      <td className="px-6 py-3.5 font-bold text-slate-900">{g.coupleNames}</td>
                      <td className="px-6 py-3.5 text-slate-600">{g.weddingDate}</td>
                      <td className="px-6 py-3.5 font-mono text-amber-700">{g.slug}</td>
                      <td className="px-6 py-3.5 text-slate-500">{g.ownerEmail}</td>
                      <td className="px-6 py-3.5 font-medium">{g.totalFiles}</td>
                      <td className="px-6 py-3.5 font-medium text-slate-600">{mb} MB</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-lg bg-white rounded-3xl shadow-2xl p-6 relative">
            ...
            <p className="text-xs text-slate-500 mb-5">
              Wypełnij podstawowe dane. Kod QR i karteczka do druku wygenerują się automatycznie.
            </p>
            ...
            {createdGallery ? (
              <div className="space-y-4">
                <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-2xl text-xs text-emerald-800 space-y-2">
                  <div className="flex items-center gap-1.5 font-bold">
                    <Check className="w-4 h-4" /> Galeria została pomyślnie utworzona!
                  </div>
                  <p>
              ...
              <form onSubmit={handleCreate} className="space-y-3 text-xs">
                <div>
                  <label className="block font-semibold text-slate-700 mb-1">Imiona Pary Młodej *</label>
                  <input
                    type="text"
                    required
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block font-semibold text-slate-700 mb-1">Data Ślubu *</label>
                    <input
                      type="date"
                      required
                      value={weddingDate}
                  ...
                  <div>
                    <label className="block font-semibold text-slate-700 mb-1">Własny Slug (opcjonalny)</label>
                    <input
                      type="text"
                ...
                <div>
                  <label className="block font-semibold text-slate-700 mb-1">E-mail Pary Młodej *</label>
                  <input
                    type="email"
                    required
                ...
                <div>
                  <label className="block font-semibold text-slate-700 mb-1">Hasło dostępu dla Pary Młodej *</label>
                  <input
                    type="password"
                    required
