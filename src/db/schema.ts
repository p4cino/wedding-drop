import { pgTable, text, timestamp, boolean, bigint, integer, uuid, index } from "drizzle-orm/pg-core";
...
export const galleries = pgTable("galleries", {
  ...
  maxStorageBytes: bigint("max_storage_bytes", { mode: "number" }).notNull().default(0), // 0 = bez limitu
...
export const cardSettings = pgTable("card_settings", {
  ...
  subheadline: text("subheadline").notNull().default("Zeskanuj kod QR aparatem w telefonie i dodaj swoje zdjęcia z naszego wesela"),
  ...
  customInstructions: text("custom_instructions").default("1. Otwórz aparat w telefonie\n2. Skieruj obiektyw na kod QR\n3. Wrzucaj zdjęcia bez rejestracji i aplikacji!"),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
});
...
export const mediaItems = pgTable(
  ...
  (table) => [
    index("idx_media_items_gallery_status_created").on(table.galleryId, table.status, table.createdAt),
