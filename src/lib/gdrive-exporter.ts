...
export async function recoverInterruptedExports() {
  try {
    ...
    console.log("Startup Recovery: Sprawdzono przerwane zadania eksportu Google Drive.");
  } catch (err) {
    console.error("Błąd podczas recovery przerwanych eksportów Google Drive:", err);
  }
}
...
export async function startGalleryDriveExport(
  ...
  if (gallery.gdriveExportStatus === "running") {
    return { success: false, alreadyRunning: true, message: "Eksport jest już w trakcie realizacji." };
  }

  // Uruchomienie w tle (bez blokowania odpowiedzi HTTP)
  runExportTask(gallery, options).catch((err) => {
    console.error(`Nieobsłużony błąd w zadaniu eksportu Google Drive dla galerii ${slug}:`, err);
  });
...
async function runExportTask(
  ...
  try {
    ...
    const allItems = await db
      .select()
      .from(mediaItems)
      .where(and(eq(mediaItems.galleryId, gallery.id), ne(mediaItems.status, "deleted")));
    ...
    if (itemsToExport.length === 0) {
      ...
      sseBus.notifyGDriveProgress(slug, { status: "completed", processedFiles: 0, totalFiles: 0 });
      return;
    }
    ...
    const bytesNeeded = unuploadedItems.reduce((sum, item) => sum + Number(item.fileSize), 0);
    ...
    const rootFolderId =
      gallery.gdriveRootFolderId || (await ensureDriveFolder(drive, rootFolderName));

    const photosFolderId =
      gallery.gdrivePhotosFolderId || (await ensureDriveFolder(drive, "Zdjęcia", rootFolderId));

    const videosFolderId =
      gallery.gdriveVideosFolderId || (await ensureDriveFolder(drive, "Filmy", rootFolderId));
    ...
    let processedFiles = itemsToExport.filter((i) => Boolean(i.gdriveFileId)).length;
    ...
    const totalBytes = itemsToExport.reduce((sum, i) => sum + Number(i.fileSize), 0);
    ...
    for (const item of itemsToExport) {
      ...
      await queue.add(async () => {
        ...
        const cleanFileName = item.originalFileName.replace(/[/\\?%*:|"<>]/g, "_");
    ...
    console.error(`Błąd podczas eksportu na Google Drive (galeria: ${slug}):`, err);
    const errorMsg = err?.message || "Wystąpił błąd podczas eksportu.";
