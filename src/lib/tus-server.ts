...
export function initTusServer(dataDir: string) {
  ...
  const server = new Server({
    ...
    onUploadCreate: async (req, upload) => {
      ...
      if (!gallerySlug) {
        throw { status_code: 400, body: "Błąd: Brak wymaganego parametru gallerySlug w metadanych." };
      }
      return { metadata };
    },
  });
  ...
  server.on(EVENTS.POST_FINISH, async (req, res, upload) => {
    ...
    if (!gallerySlug) {
      console.warn(`[TUS] Pominięto przetwarzanie uploadu ${upload.id} - brak gallerySlug`);
      return;
    }
    ...
    const isVideo = mimeType.startsWith("video") || /\.(mp4|mov|avi|webm)$/i.test(originalName);

    const tempFilePath = path.join(uploadDir, upload.id);

    console.log(`[TUS] Ukończono upload ${upload.id} dla galerii ${gallerySlug} (${upload.size} bajtów)`);
