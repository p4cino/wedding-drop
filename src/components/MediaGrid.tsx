...
import { Play, User, Image as ImageIcon } from "lucide-react";
import { MediaItemData } from "./LightboxModal";
  ...
  if (items.length === 0) {
    return (
      <div className="text-center py-20 px-4 bg-white/60 backdrop-blur-sm rounded-3xl border border-dashed border-slate-300">
        ...
        <h4 className="font-serif-luxury text-xl font-bold text-slate-800">Galeria czeka na pierwsze zdjęcia!</h4>
        <p className="text-sm text-slate-500 max-w-sm mx-auto mt-1.5">
          Bądź pierwszą osobą, która uwieczni ten wyjątkowy dzień. Kliknij przycisk poniżej, aby dodać zdjęcia.
        </p>
      </div>
    );
  }
      ...
      {items.map((item, index) => {
        ...
        return (
          <div
            ...
            className="group relative aspect-square bg-slate-100 rounded-2xl overflow-hidden cursor-pointer shadow-sm hover:shadow-md transition-all duration-300 hover:scale-[1.02]"
          >
            {/* Miniatura */}
            <img
              src={item.thumbUrl}
